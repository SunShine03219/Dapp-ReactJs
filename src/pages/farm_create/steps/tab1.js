...
export const Tab1 = (props) => {
    ...
    const selectToken = async (token) => {
        dlgMode === 0 ? setMainToken0(token) : dlgMode === 1 ? setMainToken1(token) : setRewardToken(token);
        setTokenDlgOpen(false);
    }
    ...
    const handleNextButton = () => {
        if (Object.keys(mainToken0).length === 0 || Object.keys(mainToken1).length === 0 || Object.keys(rewardToken).length === 0) {
        ...
        if (Object.keys(lpToken).length === 0 || noLP) {
            props.notify("Cannot find pair address. Add liquidity first");
            return;
        }
        props.onNext(2);
    }
    ...
    const getLPToken = async () => {
        const token0Address = mainToken0.address;

        // token 1 address, e.g. LINK token address
        const token1Address = mainToken1.address;
        try {
            ...
            const pairAddress = response.toJSON().pairAddress;
            const tokenData = await getTokenMetadata([pairAddress]);
            if (tokenData.symbol === '') {
                setNoLP(true);
    ...
    const handleForce = async () => {
        ...
        dlgMode === 0 ? setMainToken0(newToken) : dlgMode === 1 ? setMainToken1(newToken) : setRewardToken(newToken);
        setTokenDlgOpen(false);
        setSearchValue("");
    }
    ...
    useEffect(() => {
        const searchLPToken = async () => {
            if (Object.keys(mainToken0).length != 0 && Object.keys(mainToken1).length != 0) {
                setIsLoading(true);
                await getLPToken();
                setIsLoading(false);
            }
        ...
        <>
            <div className="row">
                <div className="col-6 col-xl-6" style={{ textAlign: "center", height: "90px" }}>
                    <p className='paragraph-with-bg'><strong>Select LP token pair users will stake</strong>
                    </p>
                    <input type="search" className='tab1-input-field text-center' value={Object.keys(mainToken0).length ? `${mainToken0?.symbol}(address: ${ellipseAddress(mainToken0?.address, 5)})` : ""} readOnly onClick={() => {
                        setTokenDlgOpen(true);
                        setDlgMode(0);
                    }} placeholder="Search" />
                </div>
                <div className="col-6" style={{ height: "90px", textAlign: "center" }}>
                    <p className="paragraph-with-bg" style={{ textAlign: "center" }}><strong>Select LP token pair users will stake</strong></p>
                    <input type="search" className='tab1-input-field text-center' value={Object.keys(mainToken1).length ? `${mainToken1?.symbol}(address: ${ellipseAddress(mainToken1?.address, 5)})` : ""} readOnly onClick={() => {
                        setTokenDlgOpen(true);
                        setDlgMode(1);
                    }} placeholder="Search" />
                </div>
                {isLoading && <div className="col-12 col-xl-12 align-center">
                    ...
                    <span style={{ marginLeft: "10px" }}>searching pair address ...</span>
                </div>
                }
                {
                    !isLoading && (noLP ? <div className="col-12 col-xl-12 align-center" style={{ marginBottom: "20px" }}>
                        <span>No LP found</span>
                    </div> : (Object.keys(lpToken).length > 0) && <div className="col-12 col-xl-12 align-center" style={{ marginBottom: "20px" }}>
                        <span>LP Token Address : {lpToken.address}</span>
                    </div>)
                }
                <div className="col-6 col-xl-6" style={{ textAlign: "center", height: "90px" }}>
                ...
                <div className="col-6 col-sm-12 col-xl-12 text-center-mt-6">
                    <p className='paragraph-with-bg'><strong>Daily rewards</strong><br /><br /></p>
                    <input type="number" className='tab1-input-field text-center' value={dailyReward} onChange={(e) => handleDailyReward(e.target.value)} />
                </div>
                <div className="col-12 col-xl-12 text-center-mt-6">
                    <p className='paragraph-with-bg'><strong>Total rewards</strong></p>
                    <input type="number" className='tab1-input-field text-center' value={totalReward} onChange={(e) => handleTotalReward(e.target.value)} />
                </div>
                ...
                <div className="col-xl-12 button-section"><button className="btn btn-primary next-button" data-bss-hover-animate="pulse" type="button" onClick={handleNextButton}>Next</button></div>
            </div>
                ...
                <div style={{ overflow: "auto", height: "60%" }} >
                    {
                        token_list.tokens.map((token, index) => (searchValue === "" || (token?.symbol.toLowerCase().includes(searchValue.toLowerCase()))) &&
                            <div key={index} className="token-div" style={{ display: "flex", flexDirection: "row" }} onClick={() => selectToken(token)}>
                                <img src={token.logoURI} width="28px" height="28px" style={{ borderRadius: "50%", marginRight: "12px", marginTop: '4px' }} />
